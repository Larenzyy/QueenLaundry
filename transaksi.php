...
<body>
     ...
     <div class="form-transaksi">
        <div class="input-group" style="display: flex; justify-content: flex-end; align-items: center; gap: 10px; margin-bottom: 0px;">
          <div class="form-buttons" style="transform: translateY(-18px);">
            <button id="btn-lunas" class="btn-green">Lunas</button>
            <button id="btn-belum" class="btn-red">Belum Lunas</button>
            <button id="btn-semua" class="btn-default" style="margin-left:3px;"></button>
          </div>
        ...
        <button class='btn-delete' onclick='editTransaksi($id_kasir)'>Hapus</button>
      </td>";
                    echo "</tr>";
                }
            } else {
  ...
  <script>
    ...
    document.getElementById('btn-lunas').addEventListener('click', () => filterTransaksi('Lunas'));
    document.getElementById('btn-belum').addEventListener('click', () => filterTransaksi('Belum Lunas'));
    document.getElementById('btn-semua').addEventListener('click', () => filterTransaksi('')); // tanpa filter status
    ...
    const buttons = ['btn-lunas', 'btn-belum', 'btn-semua'];
    buttons.forEach(id => {
...
let idKasirHapus = null;

function editTransaksi(id_kasir) {
  idKasirHapus = id_kasir;
...
function prosesHapus() {
  fetch(`hapus_item.php?id=${idKasirHapus}`)
    .then(res => res.text())
    .then(response => {
      if (response === 'success') {
        location.reload();
...
<div id="popup-hapus" class="popup">
  <div class="popup-content">
    <p style="margin-bottom: 20px; font-weight: 600;">Yakin ingin menghapus transaksi ini?</p>
    <div class="popup-buttons">
      <button class="btn-ya" onclick="prosesHapus()">🗑️ Hapus</button>
      <button class="btn-tidak" onclick="tutupPopupHapus()">❌ Batal</button>
    </div>
  </div>
</div>
